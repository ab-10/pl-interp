#!/usr/bin/env python3
"""Run the demo prompt with type annotations and recursive patterns features.

Focused version of demo_diff.py — only the two sharpest steering axes.
...
Usage (on GPU VM):
    python demo_diff_focused.py
    python demo_diff_focused.py --prompt "Implement a binary search function."
    python demo_diff_focused.py --temperature 0.5 --strength 5.0
"""
...
# ── Focused features: type annotations + recursive patterns ─────────────
...
def main():
    parser = argparse.ArgumentParser(
        description="Run demo prompt with type annotations & recursive patterns features")
    ...
    # Only need layer 18 SAE (both features are on layer 18)
    ...
    print(f"  Features:    Type annotations (L18:13176), Recursive patterns (L18:16290)")
    print("=" * 70)
    ...
    for feat in DEMO_FEATURES:
        ...
        print(f"  Layer:     {feat['layer']}")
        print(f"  Index:     {idx}")
        print(f"  Strength:  +{strength}")
        print("=" * 70)

        steered = steered_generate(
            model, tokenizer, sae_l18, idx, strength,
